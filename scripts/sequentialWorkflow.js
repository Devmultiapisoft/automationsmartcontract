...
class SequentialWorkflow {
    ...
    async performSingleTransfer(registeredAddress, walletId = null) {
        try {
            ...
            if (result.success) {
                // Update registered address record
                await registeredAddress.markTransferCompleted(result.txHash);

                // Also update the corresponding wallet record if walletId is provided
                if (walletId) {
                    try {
                        const wallet = await Wallet.findOne({ walletId: walletId });
                        if (wallet) {
                            await wallet.markTransferCompleted(result.txHash);
                            logger.info(`✅ Updated wallet ${walletId} transfer status to completed`);
                        } else {
                            logger.warn(`⚠️ Wallet with ID ${walletId} not found for transfer update`);
                        }
                    } catch (walletError) {
                        logger.error(`❌ Error updating wallet transfer status: ${walletError.message}`);
                        // Don't fail the transfer if wallet update fails
                    }
                }
                ...
                try {
                    ...
                    if (platformResult.success) {
                        ...
                        if (registeredAddress) {
                            ...
                            const transferResult = await this.performSingleTransfer(registeredAddress, i);
